/********************************************************************
 ...
 of doubleing point operation by a Poisson equation solver.
...
void mat_set(Matrix* Mat,int l,double z);
void mat_set_init(Matrix* Mat);
double jacobi(int n,Matrix* M1,Matrix* M2,Matrix* M3,
...
newMat(Matrix* Mat, int mnums,int mrows, int mcols, int mdeps)
{
  ...
  Mat->m= NULL;
  Mat->m= (double*) 
    malloc(mnums * mrows * mcols * mdeps * sizeof(double));
...
void
mat_set(Matrix* Mat, int l, double val)
{
  int i,j,k;
